...
# Defensive cleanup for launchers or shortcuts that accidentally pass a
# trailing quote/backslash combination. Normal launches do not need this,
# but cleaning it here prevents "Illegal characters in path" on Windows.
...
try {
    ...
    Write-Host 'VERSION 1.4 - PATH-SAFE PERMISSION REPAIR' -ForegroundColor DarkCyan
    Write-Host ''
    ...
    Show-AppMessage -Title 'Firestore rules deployed' `
        -Message "The compatible multiplayer rules were rebuilt from the current units, categories, match lengths, and player-count setting, then deployed to Firebase project:`r`n`r`n$projectId`r`n`r`nThe changes take effect immediately - no page refresh needed." `
        -Type 'Success'
}
catch {
    Write-Host ''
